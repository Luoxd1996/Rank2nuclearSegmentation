...
parser.add_argument('--data_list', type=str,
                    default='MICCAI2018/Train1', help='Name of Experiment')
parser.add_argument('--aug', type=bool, default=True,
                    help='Data augmentation')
parser.add_argument('--exp', type=str,
                    default='naive_unet', help='model_name')
...
class BCEDiceLoss(nn.Module):
    def __init__(self, mini_batch_size=5, image_sets=4):
        super(BCEDiceLoss, self).__init__()
        self.mini_batch_size = mini_batch_size
        self.image_sets = image_sets

    def _mini_batch_bce_dice_loss(self, inputs, targets):
        mini_batch_loss = 0.0
        for i in range(self.mini_batch_size):
            ...
            mini_batch_loss += bce_dice_loss
        return mini_batch_loss / self.mini_batch_size
    ...
    def forward(self, inputs, targets):
        ...
        for rounds in range(self.image_sets):
            round_loss = 0.0
            round_loss += self._mini_batch_bce_dice_loss(
                inputs[rounds], targets[rounds])
            loss += round_loss
...
if __name__ == "__main__":
    ...
    net = UNet(in_dim=3, out_dim=1, num_filter=64)
    net = net.cuda()
    if len(args.pretrained) > 0:
        net.load_state_dict(torch.load(args.pretrained), strict=False)
        print("Loaded pretrained_model!")
    print("=======> Loading dataset!")
    db_train = SegmentationData(data_list=args.data_list, aug=args.aug)
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for ind in range(batch_size):
                mini_batch_inputs, mini_batch_labels, mini_batch_classes = sampled_batch[0][ind], sampled_batch[1][ind], , sampled_batch[2][ind]
                mini_batch_seg_outputs = []
                mini_batch_cls_outputs = []

                for inp_index in range(mini_batch_inputs.size(0)):
                    inputs = mini_batch_inputs[inp_index, :, :, :].unsqueeze(
                        0).cuda()
                    ft_outputs, seg_outputs = net(inputs)
                    mini_batch_seg_outputs.append(seg_outputs)
                    mini_batch_cls_outputs.append(ft_outputs)
                cat_feature = torch.cat(mini_batch_cls_outputs, dim=0)
