...
from segmentation_dataset import ContourSegmentationData
from loss import segment_criterion, contour_criterion
...
parser.add_argument('--exp', type=str,
                    default='naive_cunet', help='model_name')
...
if __name__ == "__main__":
    ...
    net = UNet(in_dim=3, out_dim=3, num_filter=64)
    net = net.cuda()
    if len(args.pretrained) > 0:
        net.load_state_dict(torch.load(args.pretrained), strict=False)
        print("Loaded pretrained_model!")
    print("=======> Loading dataset!")
    db_train = ContourSegmentationData(
        data_list=args.data_list, patch_size=[400, 400], aug=args.aug)
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num % 50 == 0:
                ...
                predict_marker=outputs[0, 1:2, :, :]
                writer.add_image('train/predict_marker',
                                 predict_marker, iter_num)

                predict_contour=outputs[0, 2:, :, :]
                writer.add_image('train/predict_contour',
                                 predict_contour, iter_num)
